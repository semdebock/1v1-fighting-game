/* Fight Arena v0.9.5 — Campaign Phases Foundation stable loader */
(async()=>{
  ...
  try{
    ...
    let code='';
    for(const file of files){const r=await fetch(`build/v093/${file}?v=095`,{cache:'no-store'});if(!r.ok)throw new Error(`${file} HTTP ${r.status}`);code+=await r.text()}
    ...
    console.error('[Fight Arena v0.9.5 loader]',err);
    const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.5 • CORE LOAD ERROR';
