...
function brand(){
  ...
  document.querySelectorAll('.brand .tag').forEach(x=>x.textContent=VERSION);
  const update=$('#updates');if(update)update.textContent=`UPDATE LOG  •  ${VERSION}`;
  const latest=$('.latest-stat');if(latest){const b=$('b',latest),s=$('span',latest);if(b)b.textContent=VERSION;if(s)s.textContent='Gameplay & UX • Main Menu Rework • iPad Polish'}
  const health=$('.build-health b');if(health)health.textContent=`${VERSION} • DEV • UX PASS`;
  const chooser=$('#deviceChooser .eyebrow');if(chooser)chooser.textContent=`MULTIVERSE ARENA • ${VERSION}`;
  const dock=$('#ownerBoardDock small');if(dock)dock.textContent=VERSION;
  const ownerSetting=$('#ownerBoardLaunch')?.closest?.('.setting-row')?.querySelector('b');if(ownerSetting)ownerSetting.textContent=`👑 OWNER BOARD • ${VERSION}`;
}
function refreshHome(){
  const copy=$('#home .hero-copy');
  if(copy){const tag=$('.tag',copy),h=$('h1',copy),p=$('p',copy);if(tag)tag.textContent='v0.9.8 • GAMEPLAY & UX';if(h)h.innerHTML='FIGHT.<br>COLLECT.<br>CONQUER.';if(p)p.textContent='Choose your fighter, jump into the Villain Gauntlet and manage your roster with a faster, cleaner iPad-first interface.'}
  const dashTitle=$('#home .hero-dashboard .dash-title strong');if(dashTitle)dashTitle.textContent='READY FOR BATTLE';
}
function updateLog(){
  ...
  box.innerHTML=rows.map(([i,t,p])=>`<div class="log-item"><div class="log-icon">${i}</div><div><b>${t}</b><p>${p}</p></div></div>`).join('');
...
const mo=new MutationObserver(()=>{if($('#ownerBoardDock small')?.textContent!==VERSION)brand()});
