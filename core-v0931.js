/* Fight Arena v0.9.3.1 — DIAMONDS redeem patch loader */
(async()=>{
  const files=['core-v093-part1.txt','core-v093-part2.txt','core-v093-part3.txt','core-v093-part4.txt'];
  try{
    let code='';
    for(const file of files){const r=await fetch(`build/v093/${file}?v=0931`,{cache:'no-store'});if(!r.ok)throw new Error(`${file} HTTP ${r.status}`);code+=await r.text()}
    code=code.replace('/* Fight Arena v0.9.3 — Power & Balance Update */','/* Fight Arena v0.9.3.1 — DIAMONDS Redeem Patch */');
    code=code.replace("window.FightArena={version:'0.9.3'","window.FightArena={version:'0.9.3.1'");
    ...
    code=code.replace("s.coreVersion='0.9.3'","s.coreVersion='0.9.3.1'");
    ...
    console.error('[Fight Arena v0.9.3.1 loader]',err);
    const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.3.1 • CORE LOAD ERROR';
