/* Fight Arena v0.9.5.3.1 — Result Flow & Unlock Polish stable loader */
(async()=>{
 ...
 try{
  ...
  if(typeof window.FightArenaTransformV0953!=='function')throw new Error('v0.9.5.3 transform unavailable');
  if(typeof window.FightArenaTransformV09531!=='function')throw new Error('v0.9.5.3.1 transform unavailable');
  let code='';
  for(const file of files){const r=await fetch(`build/v093/${file}?v=09531`,{cache:'no-store'});if(!r.ok)throw new Error(`${file} HTTP ${r.status}`);code+=await r.text()}
  ...
  console.error('[Fight Arena v0.9.5.3.1 loader]',err);
  const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.5.3.1 • CORE LOAD ERROR';
