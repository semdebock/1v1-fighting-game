/* Fight Arena v0.9.5.3 — Vigilante Line stable loader */
(async()=>{
 ...
 try{
  ...
  let code='';
  for(const file of files){const r=await fetch(`build/v093/${file}?v=0953`,{cache:'no-store'});if(!r.ok)throw new Error(`${file} HTTP ${r.status}`);code+=await r.text()}
  ...
  console.error('[Fight Arena v0.9.5.3 loader]',err);
  const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.5.3 • CORE LOAD ERROR';
