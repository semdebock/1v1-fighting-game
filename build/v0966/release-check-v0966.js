...
for(const marker of ["const BUILD='0.9.6.6'","const ASSET='0966'","const SAVE_KEY='fightArenaV08'","'update-v0966.js'"])if(!boot.includes(marker))throw new Error('v0.9.6.6 bootstrap marker missing: '+marker);
if(boot.includes("'update-v0965.js'"))throw new Error('legacy v0.9.6.5 UI runtime is double-loaded');
for(const marker of ['function phaseAccessible','function renderNavigator','function nearestIndex','Phase Tabs • Swipe Villains • Smart Target Focus','SWIPE TO CHOOSE','CLASSIFIED THREAT','window.MultiverseArenaUpdate0966','scroll-snap'])if(!ui.includes(marker)&&!css.includes(marker))throw new Error('v0.9.6.6 navigator marker missing: '+marker);
...
console.log('v0.9.6.6 swipeable Gauntlet navigator checks passed');
