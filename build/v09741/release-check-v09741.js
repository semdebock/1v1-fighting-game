...
for(const m of ["const SAVE_KEY='fightArenaV08'","'update-v09741.js'",'core-runtime-v0958.js'])if(!boot.includes(m))throw new Error('v0.9.7.4.1 compatibility bootstrap marker missing '+m);
if(!/const BUILD='0\.9\.7\.(?:4\.1|[5-9]|\d{2,})(?:\.\d+)?'/.test(boot))throw new Error('v0.9.7.4.1 compatibility requires current 0.9.7.4.1+ bootstrap');
if(!/const ASSET='097(?:41|[5-9]\d*)'/.test(boot))throw new Error('v0.9.7.4.1 compatibility requires current 09741+ asset');
for(const m of ['window.MultiverseArenaUpdate09741','restoreRematch','rematchWired09741','FightArenaCampaignControls','const ownsRelease='])if(!update.includes(m))throw new Error('v0.9.7.4.1 compatibility runtime marker missing '+m);
...
if(!oldUpdate.includes('const ownsRelease='))throw new Error('v0.9.7.4 presentation can overwrite newer releases');
...
console.log('v0.9.7.4.1 Hulkbuster rework + Rematch compatibility checks passed');
