...
return function transformV0976(code){
 ...
 rep("if(l.boss){const intro=document.createElement('div');intro.className='boss-intro';intro.innerHTML=`<div><small>LEVEL ${l.n} • BOSS ENCOUNTER</small><strong>${l.name.toUpperCase()}</strong><span>DEFEAT THE BOSS • 💎 ${l.gems}</span></div>`;$('arena').appendChild(intro);later(()=>intro.remove(),1400)}", "$('bossPhaseIndicatorV0976')?.remove();if(l.boss){const intro=document.createElement('div');intro.className='boss-intro boss-intro-v0976';intro.innerHTML=`<div><small>◆ BOSS ENCOUNTER • LEVEL ${l.n}</small><strong>${l.name.toUpperCase()}</strong><span>BOSS REWARD • 🪙 ${l.coins} • ⭐ ${l.xp} XP • 💎 ${l.gems}</span></div>`;$('arena').appendChild(intro);if(l.name==='Doctor Octopus'||l.name==='Magneto')updateBossPhaseV0976('PHASE 1',1,3);later(()=>intro.remove(),1650)}", 'premium boss intro and clean phase start');
