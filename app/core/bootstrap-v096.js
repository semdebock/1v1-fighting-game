/* Multiverse Arena v0.9.7.1 — Sabretooth Rework + Stability Bootstrap */
(()=>{
'use strict';
const BUILD='0.9.7.1';
const ASSET='0971';
...
const RUNTIME=['device-v092.js','touch-v0941.js','stability-v0941.js','campaign-v097.js','ui-v09571.js','polish-v09572.js','premium-v096.js','update-v0962.js','update-v0963.js','update-v0964.js','update-v097.js','update-v0971.js'];
...
function brand(){document.title='Multiverse Arena v0.9.7.1 — Sabretooth Rework + Stability';document.querySelectorAll('.brand .tag').forEach(x=>x.textContent='v0.9.7.1');const updates=document.getElementById('updates');if(updates)updates.textContent='UPDATE LOG  •  v0.9.7.1';const latest=document.querySelector('.latest-stat');if(latest){const b=latest.querySelector('b'),s=latest.querySelector('span');if(b)b.textContent='v0.9.7.1';if(s)s.textContent='Sabretooth Rework • Result Flow • Version Sync'}const health=document.querySelector('.build-health b');if(health)health.textContent='v0.9.7.1 • SABRETOOTH + RESULT STABILITY • STABLE';const chooser=document.querySelector('#deviceChooser .eyebrow');if(chooser)chooser.textContent='MULTIVERSE ARENA • v0.9.7.1';const pause=document.querySelector('#pauseOverlay .eyebrow');if(pause)pause.textContent='MULTIVERSE ARENA'}
function fatal(err){state.status='error';console.error('[Multiverse Arena v0.9.7.1 bootstrap]',err);const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.7.1 • CORE LOAD ERROR';const t=document.getElementById('toast');if(t){t.textContent='CORE LOAD ERROR — REFRESH';t.classList.remove('hidden')}}
function validateReady(){const missing=[];if(window.FightArena?.version!=='0.9.5.7')missing.push('FightArena core');if(!window.FightArenaControls)missing.push('controls');if(!window.FightArenaCampaignV097)missing.push('Mutant Uprising campaign');if(!window.MultiverseArenaUI)missing.push('UI');if(!window.MultiverseArenaPolish)missing.push('combat polish');if(!window.MultiverseArenaPremium)missing.push('premium presentation');if(!window.MultiverseArenaUpdate0962)missing.push('v0.9.6.2 update');if(!window.MultiverseArenaUpdate0963)missing.push('v0.9.6.3 update');if(!window.MultiverseArenaUpdate0964)missing.push('v0.9.6.4 update');if(!window.MultiverseArenaUpdate097)missing.push('v0.9.7 campaign navigator');if(!window.MultiverseArenaUpdate0971)missing.push('v0.9.7.1 stability layer');if(missing.length){state.status='warning';state.missing=missing;console.warn('[Multiverse Arena v0.9.7.1] runtime warning',missing)}else state.status='ready';setTimeout(brand,320)}
...
async function boot(){try{
 ...
 setTimeout(()=>{if(state.status==='booting')validateReady()},420);
}catch(err){fatal(err)}}
boot();
})();
