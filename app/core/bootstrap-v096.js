/* Multiverse Arena v0.9.6.5 — Villain Gauntlet Focus Bootstrap */
(()=>{
'use strict';
const BUILD='0.9.6.5';
const ASSET='0965';
...
const RUNTIME=['device-v092.js','touch-v0941.js','stability-v0941.js','campaign-v0957.js','ui-v09571.js','polish-v09572.js','premium-v096.js','update-v0962.js','update-v0963.js','update-v0964.js','update-v0965.js'];
...
function brand(){document.title='Multiverse Arena v0.9.6.5 — Villain Gauntlet Focus';document.querySelectorAll('.brand .tag').forEach(x=>x.textContent='v0.9.6.5');const updates=document.getElementById('updates');if(updates)updates.textContent='UPDATE LOG  •  v0.9.6.5';const latest=document.querySelector('.latest-stat');if(latest){const b=latest.querySelector('b'),s=latest.querySelector('span');if(b)b.textContent='v0.9.6.5';if(s)s.textContent='Gauntlet Focus • Camera Slides • Classified Threats'}const health=document.querySelector('.build-health b');if(health)health.textContent='v0.9.6.5 • GAUNTLET CLARITY • STABLE';const chooser=document.querySelector('#deviceChooser .eyebrow');if(chooser)chooser.textContent='MULTIVERSE ARENA • v0.9.6.5';const pause=document.querySelector('#pauseOverlay .eyebrow');if(pause)pause.textContent='MULTIVERSE ARENA'}
function fatal(err){state.status='error';console.error('[Multiverse Arena v0.9.6.5 bootstrap]',err);const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.6.5 • CORE LOAD ERROR';const t=document.getElementById('toast');if(t){t.textContent='CORE LOAD ERROR — REFRESH';t.classList.remove('hidden')}}
function validateReady(){const missing=[];if(window.FightArena?.version!=='0.9.5.7')missing.push('FightArena core');if(!window.FightArenaControls)missing.push('controls');if(!window.FightArenaCampaignV0957)missing.push('Villain Gauntlet');if(!window.MultiverseArenaUI)missing.push('UI');if(!window.MultiverseArenaPolish)missing.push('combat polish');if(!window.MultiverseArenaPremium)missing.push('premium presentation');if(!window.MultiverseArenaUpdate0962)missing.push('v0.9.6.2 update');if(!window.MultiverseArenaUpdate0963)missing.push('v0.9.6.3 update');if(!window.MultiverseArenaUpdate0964)missing.push('v0.9.6.4 update');if(!window.MultiverseArenaUpdate0965)missing.push('v0.9.6.5 update');if(missing.length){state.status='warning';state.missing=missing;console.warn('[Multiverse Arena v0.9.6.5] runtime warning',missing)}else state.status='ready';setTimeout(brand,220)}
...
async function boot(){try{
 ...
 await script('update-v0965.js');
 await waitFor(()=>window.MultiverseArenaUpdate0965,2500,'v0.9.6.5 update');
 window.dispatchEvent(new Event('fightarena-ready'));
 setTimeout(()=>{if(state.status==='booting')validateReady()},320);
}catch(err){fatal(err)}}
boot();
})();
