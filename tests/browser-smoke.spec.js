...
test('Multiverse Arena boots v0.9.8 and exposes critical controls', async ({ page }) => {
  const errors=[];
  ...
  await page.waitForFunction(() => window.MultiverseArenaUpdate098?.version === '0.9.8', null, { timeout:10000 });
  await expect(page.locator('.brand .tag').first()).toContainText('0.9.8');
