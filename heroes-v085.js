...
(() => {
  ...
  // IMPORTANT: base game uses `price`, `persist()` and cls() for sprite class names.
  chars['Captain America']={price:2200,hp:115,power:'High',special:'Shield Throw',dmg:1.05,speed:.96,cls:'captain',role:'Tactical',rank:'A',desc:'Tactical shield fighter. Shield Throw ricochets back, Shield Bash closes distance and Guard Stance reduces incoming pressure.'};
  chars['Iron Man']={price:3200,hp:105,power:'Very High',special:'Unibeam',dmg:1.12,speed:1.04,cls:'ironman',role:'Tech / Flight',rank:'S',desc:'Premium ranged hero. Repulsor Blast, Micro Missile and temporary Flight give Iron Man the deepest ability kit in the current roster.'};

  // Extend the base sprite-class mapper so names with spaces get valid CSS classes.
  const baseCls = cls;
  cls = function(type){
    if(type==='Iron Man') return 'ironman';
    if(type==='Captain America') return 'captain';
    return baseCls(type);
  };
  ...
  if(!save.v085OwnershipFixed){
    ...
    save.owned['Captain America']=false;
    save.owned['Iron Man']=false;
    save.v085OwnershipFixed=true;
  }
  persist();
  ...
  function stampVersion(){
    ...
    const intro=document.querySelector('#home .hero-copy p'); if(intro) intro.textContent='Iron Man and Captain America join the roster with unique abilities and ranked hero kits.';
  }
  stampVersion();
  ...
  if(logPanel) logPanel.innerHTML=`<span class="tag">v0.8.5 • HERO EXPANSION</span><h2>ASSEMBLE YOUR HERO ROSTER.</h2><div class="changelog">
    <div class="log-item"><div class="log-icon">🛡️</div><div><b>Captain America • A Rank • 2,200 coins</b><p>115 HP tactical fighter with Shield Throw, Shield Bash and Guard Stance.</p></div></div>
    <div class="log-item"><div class="log-icon">🤖</div><div><b>Iron Man • S Rank • 3,200 coins</b><p>105 HP premium tech fighter with Repulsor Blast, Micro Missile, Flight and Unibeam.</p></div></div>
    <div class="log-item"><div class="log-icon">🧰</div><div><b>v0.8.5 compatibility hotfix</b><p>Fixed hero prices, profile stats, ownership, version labels and Iron Man/Captain America sprite class mapping.</p></div></div>
  </div>`;

  renderChars();
  chosen = save.selected;
  renderCharInfo();
  hud();
  stampVersion();
