/* Fight Arena v0.9.5.6 — Goblin Rework & Training Lab stable loader */
(async()=>{
 const files=['core-v093-part1.txt','core-v093-part2.txt','core-v093-part3.txt','core-v093-part4.txt'];
 try{
  for(const [name,fn] of Object.entries({v094:window.FightArenaTransformV094,v0941:window.FightArenaTransformV0941,v095:window.FightArenaTransformV095,v0951:window.FightArenaTransformV0951,v0952:window.FightArenaTransformV0952,v0953:window.FightArenaTransformV0953,v09531:window.FightArenaTransformV09531,v0954:window.FightArenaTransformV0954,v0955:window.FightArenaTransformV0955,v0956:window.FightArenaTransformV0956}))if(typeof fn!=='function')throw new Error(name+' transform unavailable');
  let code='';
  for(const file of files){const r=await fetch(`build/v093/${file}?v=0956`,{cache:'no-store'});if(!r.ok)throw new Error(`${file} HTTP ${r.status}`);code+=await r.text()}
  ...
  console.error('[Fight Arena v0.9.5.6 loader]',err);
  const h=document.querySelector('.build-health b');if(h)h.textContent='v0.9.5.6 • CORE LOAD ERROR';
